...
# from vllm_client import LLMClient,Message
...
# client = LLMClient(
# api_url="http://10.176.64.144:59824/v1/chat/completions",
# model_name="/data/hjy/models/Qwen2.5-14B-Instruct"
# )
...
# def reward_correct(contents):
    ...
    # prompts = []
    # for x in contents:
    #     matches = re.findall(r'\\boxed\{(.*?)\}', x['answer'])
    #     final_answer =  "\\boxed{" + matches[0] + "}" if matches else x['answer']
    #     question = [Message(role="system", content=x['sys_prompt']),Message(role="user", content=usr_prompt.format(Question = x['q'], Ground_Truth = x['std'], Answer = final_answer))]
...
model_path = "/mnt/data/kw/models/DeepSeek-R1-Distill-Qwen-7B"
if 'ref' in sys.argv:
...
if __name__ == '__main__':
    ...
    #     QAs = json.load(f)
    with open("/mnt/data/kw/hjy/data/dapo-math/dapo_math.json", 'r', encoding='utf-8') as file:
    ...
    lsrl = LSRL(model_path, epochs=1, train_data=QAs, 
                big_rollout_num = 4, 
                rollout_num=2, 
                bad_num = 1,
                train_batch_size=2, gen_batch_size = 10,
                gen_update_steps=16, trainer='LSCPU', gen_temperature=1,
                gen_device=[3], ref_server="http://localhost:59887",
                lr=1e-6, 
                accum_steps=64, 
                swanlab_project="fast_slow_thinking",
                swanlab_name="0827_7B_concise",
                gen_max_tokens=4096,
                save_steps=500,
                save_path_fold ="/mnt/data/kw/hjy/ckp/0827/7B_concise",
                eval_llm_flag = False,
                genlog_filename = "/mnt/data/kw/hjy/logs/train/0827/7B_concise"
                )
