...
class LSRL:
    def __init__(self, model_path, epochs=1, algorithm="GRPO", rollout_num=8, train_data=None, trainer='LSCPU',
                 ...
                 swanlab_project = None, swanlab_name = None,eval_llm_flag=True,
                 **kwargs):
...
# V2:
    ...
    #         rec_resps = self.generate_tool_insert(model, recursive_prompts, num+1, group_expressions, group_numbers)
    #         for org_id, rec_resp in zip(recursive_ids, rec_resps):
    #             responses[org_id] += rec_resp
    #     return responses

    
    # def generate_tool_insert(self, vllm_gen, tool_prompts, gen_number, call_number, group_expressions, group_numbers):
    #     function_call_res = "The result of executing this funciton is:"
    #     stop_sentences = "</function>"
    #     from vllm import SamplingParams
    #     sampling_params = SamplingParams(
    ...
    #         include_stop_str_in_output=True
    #     )
    #     voutputs = vllm_gen.generate(tool_prompts, sampling_params, use_tqdm=False)

    #     # 每个 prompt 对应 gen_number 个答案
    #     all_answers = []  # List[List[Dict]], 每个元素是一组对应 prompt 的答案
    #     for v in voutputs:
    #         group = []
    #         for z in v.outputs:
    #             group.append(z.text)
    ...
    #     if call_number > 8:
    #         # 直接扁平化并返回
    #         return [ans for group in all_answers for ans in group]

    #     recursive_prompts = []
    #     recursive_indices = []  # 存储 (prompt_idx, gen_idx)
    #     for i, group in enumerate(all_answers):  # 对每个prompt
    #         for j, ans in enumerate(group):      # 对该prompt的每个生成结果
    #             if ans.endswith(stop_sentences):
    #                 # 工具调用
    #                 computed_result = run(ans, group_expressions[i], group_numbers[i])
    #                 # 融合结果
    #                 ans += f"\n{function_call_res}{computed_result}"
    #                 # 准备新的 prompt 用于递归调用
    #                 new_prompt = tool_prompts[i] + ans
    #                 recursive_prompts.append(new_prompt)
    #                 recursive_indices.append((i, j))

    #     if recursive_prompts:
    #         # 递归调用：因为是一个flat list，gen_number=1
    #         rec_results = self.generate_tool_insert(
    #             vllm_gen, recursive_prompts, gen_number, call_number=call_number + 1,
    #             group_expressions=[group_expressions[i] for i, _ in recursive_indices],
    #             group_numbers=[group_numbers[i] for i, _ in recursive_indices]
    #         )

    #         # 将递归的结果合并到对应答案中
    #         for (i, j), rec_group in zip(recursive_indices, rec_results):
    #             rec_ans = rec_group  
    #             all_answers[i][j]+= rec_ans
             
    #     # 最终扁平化结果：List
    #     flat_answers = [ans for group in all_answers for ans in group]
    #     return flat_answers
    ...
    def gen_worker(self, Q_data, Q_state_dict, gen_device, gen_rank=0):
        ...
        def gen_samples(items):
            ...
            # for i in range(gen_tool_num_single):
            #     tool_answers_temp = self.generate_tool_insert(vllm_gen, tool_prompts, 0, group_expressions, group_numbers)
            #     tool_answers_group.append(tool_answers_temp)
            # print(f"group的长度:{len(tool_answers_group)}")
            # tool_answers = [x for group in zip(*tool_answers_group) for x in group]
            tool_answers = self.generate_tool_insert(vllm_gen, tool_prompts, 0, group_expressions, group_numbers)
            # print(f"tool answer的长度：{len(tool_answers)}") 
            # print(f"!!!tool_prompts:{len(tool_prompts)} items length:{len(items)}")
            answers = []
            group1 = [normal_answers[i:i+gen_normal_num_single] for i in range(0, len(normal_answers), gen_normal_num_single)]
            
            # group2 = [tool_answers[i,i+len(items)] for i in range(0, len(tool_answers), len(items))]
            # group2 = tool_answers
            group2 = []
