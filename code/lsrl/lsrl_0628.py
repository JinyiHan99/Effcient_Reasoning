...
class LSRL:
    ...
    def GRPO_step(self, model, batch):
        ...
        #后面的步骤逐步加上对cost的惩罚
        if os.path.isdir(f"{self.save_path_fold}/step_900"):
            print("!!!900步后加上对cost的惩罚")
            reward = (batch['acc'] + batch['format'] + batch['cost']).to(self.device)
        #前300步只计算两项 
        else:
            reward = (batch['acc'] + batch['format']).to(self.device)
            print("!!!no cost")
       
        advantages = (reward - reward.mean()) / (reward.std() + 1e-4)
